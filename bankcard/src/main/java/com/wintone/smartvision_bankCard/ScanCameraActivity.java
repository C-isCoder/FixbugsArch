...
public class ScanCameraActivity extends AppCompatActivity implements Callback, PreviewCallback {
    ...
    private void findView() {
        ...
        this.copyright_label = findViewById(R.id.copyright_label);
        ...
        LayoutParams layoutParams = new LayoutParams(back_w, back_w);
        ...
        layoutParams.leftMargin =
            (int) ((((((double) this.width) - ((((double) Fheight) * 0.8d) * 1.585d)) / 2.0d)
                - ((double) back_w)) / 2.0d);
        ...
        this.flash.setOnClickListener(new OnClickListener() {
            public void onClick(View v) {
                if (!ScanCameraActivity.this.getPackageManager()
                    .hasSystemFeature("android.hardware.camera.flash")) {
                    Toast.makeText(ScanCameraActivity.this, ScanCameraActivity.this.getResources()
                            .getString(ScanCameraActivity.this.getResources()
                                .getIdentifier("toast_flash", "string",
                                    ScanCameraActivity.this.getApplication().getPackageName())),
                        Toast.LENGTH_SHORT)
                        .show();
                    ...
                    try {
                        ...
                        Toast.makeText(ScanCameraActivity.this,
                            ScanCameraActivity.this.getResources()
                                .getString(ScanCameraActivity.this.getResources()
                                    .getIdentifier("toast_flash", "string",
                                        ScanCameraActivity.this.getApplication().getPackageName())),
        ...
        if (this.camera == null) {
            try {
                ...
                Toast.makeText(getApplicationContext(),
                    getResources().getString(R.string.toast_camera), Toast.LENGTH_SHORT).show();
                return;
            }
        }
        try {
    ...
    public void onPreviewFrame(byte[] data, Camera camera) {
        ...
        if (this.counter == 2) {
            ...
            int result = this.api.RecognizeNV21(data, parameters.getPreviewSize().width,
                parameters.getPreviewSize().height, isBorders, recogval, 30, bRotated, pLineWarp);
            if (isBorders[0] == 1) {
                if (this.myView != null) {
            ...
            if (isBorders[0] != 1 || isBorders[1] != 1 || isBorders[2] != 1 || isBorders[3] != 1) {
                ...
                intent.putExtra(IMAGE_DATA, pLineWarp);
                intent.putExtra(CARD_DATA, recogval);
    ...
    public void getCameraPreParameters(Camera camera) {
        ...
        if ("PLK-TL01H".equals(Build.MODEL)) {
            ...
            while (i < this.list.size()) {
                if (ratioScreen == ((float) (this.list.get(i)).width) / ((float) (this.list.get(
                    i)).height) && ((this.list.get(i)).width >= 1280
                    || (this.list.get(i)).height >= 720)) {
                    ...
                    if ((this.list.get(0)).width > (this.list.get(this.list.size() - 1)).width) {
                        if (this.preWidth > (this.list.get(i)).width
                            || this.preHeight > (this.list.get(i)).height) {
                            this.preWidth = (this.list.get(i)).width;
                            this.preHeight = (this.list.get(i)).height;
                        }
                    } else if ((this.preWidth < (this.list.get(i)).width
                        || this.preHeight < (this.list.get(i)).height)
                        && this.preWidth < 1280
                        && this.preHeight < 720) {
                        this.preWidth = (this.list.get(i)).width;
            ...
            if (!this.isShowBorder) {
                ...
                this.surfaceWidth = (int) ((((float) this.preWidth) / ((float) this.preHeight))
                    * ((float) this.srcHeight));
                this.surfaceHeight = this.srcHeight;
            } else {
                this.surfaceWidth = this.srcWidth;
                this.surfaceHeight = (int) ((((float) this.preHeight) / ((float) this.preWidth))
                    * ((float) this.srcHeight));
            }
        }
    }
